...
function ensureSymlink(targetValue, targetPath, type) {
  const trySymlink = () => { fs.symlinkSync(targetValue, targetPath, type); };
  const getResolvedTarget = () => path.resolve(path.dirname(targetPath), targetValue);
  ...
  try {
    ...
    if (error?.code === "EPERM" && process.platform === "win32") {
      const resolvedSource = getResolvedTarget();
