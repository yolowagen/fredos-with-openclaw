...
describe("resolveEffectiveToolPolicy", () => {
  it("defaults the main agent to the coding-project profile when no tool profile is configured", () => {
    const cfg = {
      agents: {
    ...
    expect(result.profile).toBe("coding-project");
  });

  it("does not force coding-project onto non-default agents without an explicit profile", () => {
    const cfg = {
      agents: {
        list: [
    ...
    const result = resolveEffectiveToolPolicy({ config: cfg, agentId: "research" });
